...
const unsigned short* wordchars_utf16 = nullptr;
std::vector<unsigned short> new_wordchars_utf16;
...
TextParser* get_parser(int format, const char* extension, Hunspell* pMS) {
  ...
  if (io_utf8) {
    const std::vector<unsigned short>& vec_wordchars_utf16 = pMS->get_wordchars_utf16();
...
#ifdef UCSSPELL_LOG_FILE
 init_ucspell_logging();
#endif
  std::string buf;
  Hunspell* pMS[DMAX];
  char* key = nullptr;
